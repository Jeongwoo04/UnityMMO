...
bool DBConnection::BindParamOut(int32 paramIndex, bool* value, SQLLEN* index)
{
	return BindParamOut(paramIndex, SQL_C_TINYINT, SQL_TINYINT, size32(bool), value, index);
}

bool DBConnection::BindParamOut(int32 paramIndex, float* value, SQLLEN* index)
{
	return BindParamOut(paramIndex, SQL_C_FLOAT, SQL_REAL, 0, value, index);
}

bool DBConnection::BindParamOut(int32 paramIndex, double* value, SQLLEN* index)
{
	return BindParamOut(paramIndex, SQL_C_DOUBLE, SQL_DOUBLE, 0, value, index);
}

bool DBConnection::BindParamOut(int32 paramIndex, int8* value, SQLLEN* index)
{
	return BindParamOut(paramIndex, SQL_C_TINYINT, SQL_TINYINT, size32(int8), value, index);
}

bool DBConnection::BindParamOut(int32 paramIndex, int16* value, SQLLEN* index)
{
	return BindParamOut(paramIndex, SQL_C_SHORT, SQL_SMALLINT, size32(int16), value, index);
}

bool DBConnection::BindParamOut(int32 paramIndex, int32* value, SQLLEN* index)
{
	return BindParamOut(paramIndex, SQL_C_LONG, SQL_INTEGER, size32(int32), value, index);
}

bool DBConnection::BindParamOut(int32 paramIndex, int64* value, SQLLEN* index)
{
	return BindParamOut(paramIndex, SQL_C_SBIGINT, SQL_BIGINT, size32(int64), value, index);
}

bool DBConnection::BindParamOut(int32 paramIndex, TIMESTAMP_STRUCT* value, SQLLEN* index)
{
	return BindParamOut(paramIndex, SQL_C_TYPE_TIMESTAMP, SQL_TYPE_TIMESTAMP, size32(TIMESTAMP_STRUCT), value, index);
}

bool DBConnection::BindParamOut(int32 paramIndex, const WCHAR* str, SQLLEN* index)
{
	SQLULEN size = static_cast<SQLULEN>((::wcslen(str) + 1) * 2);
	*index = SQL_NTSL;

	if (size > WVARCHAR_MAX)
		return BindParamOut(paramIndex, SQL_C_WCHAR, SQL_WLONGVARCHAR, size, (SQLPOINTER)str, index);
	else
		return BindParamOut(paramIndex, SQL_C_WCHAR, SQL_WVARCHAR, size, (SQLPOINTER)str, index);
}

bool DBConnection::BindParamOut(int32 paramIndex, const BYTE* bin, int32 size, SQLLEN* index)
{
	if (bin == nullptr)
	{
		*index = SQL_NULL_DATA;
	...
	if (size > BINARY_MAX)
		return BindParamOut(paramIndex, SQL_C_BINARY, SQL_LONGVARBINARY, size, (BYTE*)bin, index);
	else
		return BindParamOut(paramIndex, SQL_C_BINARY, SQL_BINARY, size, (BYTE*)bin, index);
}
...
bool DBConnection::BindParamOut(SQLUSMALLINT paramIndex, SQLSMALLINT cType, SQLSMALLINT sqlType, SQLULEN len, SQLPOINTER ptr, SQLLEN* index)
{
...
//bool DBConnection::BindCol(SQLUSMALLINT columnIndex, SQLSMALLINT cType, SQLULEN len, SQLPOINTER value, SQLLEN* index) {
//	// SQL_C_WCHAR로 설정해 UTF-16 문자열을 올바르게 처리
//	SQLRETURN ret = ::SQLBindCol(_statement, columnIndex, cType, value, len, index);
//	if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO) {
//		HandleError(ret);
//		return false;
